 ...
 * @author xiongzl
 **/
public class TestNG_Report_ScreenShot {
    ...
    private void loginTest() throws InterruptedException {
        ...
        // 输入用户名
        webDriver.findElement(By.xpath("//div[@class='u-input box']/input[@name='email']")).sendKeys("xiongzl66");
    ...
    @AfterMethod
    private void afterClass(ITestResult result) throws IOException {
        if (result.getStatus() == ITestResult.FAILURE) {
            // 执行截图操作
            String imgPath = Utils.takeScreenShot(webDriver, result.getName());
            final String screenShotPath = test.addScreenCapture(imgPath);
            test.log(LogStatus.FAIL, "登陆失败", screenShotPath);
        }
        webDriver.quit();
        report.endTest(test);
        report.flush();
